...
int dealloc_merge(int i, int mode) {
	...
	for (j = 0; j < __num_chunk__ + 1; j++) {
		if (mlist[j].avail == 0)
			continue;
		else if (mlist[j].base + mlist[j].size == mlist[i].base) {
...
int init_alloc() {
	...
	mem_init_loc = mmap(0, mem_for_struct_m_list, PROT_READ | PROT_WRITE,
	...
	mem_use_loc = mmap(0, PAGESIZE, PROT_READ | PROT_WRITE,
			MAP_PRIVATE | MAP_ANONYMOUS, 0, 0);
	if (mem_init_loc == MAP_FAILED) {
		perror("mmap failed\n");
		return 1;
...
int cleanup() {
	...
	stat_check = munmap(mem_init_loc,
			(PAGESIZE / MINALLOC) * sizeof(m_list) + PAGESIZE);
	if (stat_check == -1)
		return 1;
	return 0;
}
