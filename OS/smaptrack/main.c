...
void InitSmaptrackBlock(struct smaptrack_block *sb) {
    memset(sb->lines, 0x00, __SMAPTRACK_BLOCK_LINE_MAXIMUM__ * __SMAPTRACK_LINE_LENGTH__);
}
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 2) {
        ...
        while (iter < argc-1) {
            ...
            if ((c_p = strstr(argv[iter], "--grep")) != NULL) {
                c_p = strstr(c_p, "=");
                snprintf(grep_block[grep_block_count], sizeof(grep_block[grep_block_count]), "%s", c_p+1);
                ...
                c_p = strstr(c_p, "=");
                char buffer[100];
                snprintf(buffer, sizeof(buffer), "%s", c_p+1);
    ...
    memset(dirloc, 0x00, sizeof(dirloc));

    snprintf(dirloc, sizeof(dirloc),  "/proc/%d/smaps", atoi(argv[1]));
    ...
    while (1) {
        ...
        while (i < read_size) {
            if (string_buffer[i] == '\n') {
                ...
                buffer[b_i] = '\0';
                strncpy(smapsb[count].lines[j], buffer, sizeof(smapsb[count].lines[j]));
                    ...
                    if (strcmp(smapsb[i].lines[j], smapsbprev[i].lines[j])) {
                        ...
                        if (match == 1) {
                            ...
                            for (j = 0; j < smap_elem_line_count; j++) {
                                ...
                                strncpy(smapsbprev[i].lines[j], smapsb[i].lines[j], sizeof(smapsbprev[i].lines[j]));
                            }
                            fprintf(smaptrack_file, "\n--------------------------------------------------\n");
                        }
