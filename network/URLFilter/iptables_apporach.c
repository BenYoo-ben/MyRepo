...
int MiniDigSendQuery(char *s, char *dns_server_addr) {
    ...
    int16_t addr_len = sizeof(struct sockaddr_in);

    struct sockaddr_in dns_addr;
    memset(&dns_addr, 0x00, addr_len);
    ...
    if (rv == 0) {
        ...
        while (found_ip_count < answer_count) {
            ...
            if ((c & (0xc0)) == (0xc0)) {
                ...
                c = recv_buf[i];
                snprintf(mini_dig_ips[found_ip_count],
                        elemSize, "%u.", c);
                i++;
                c = recv_buf[i];
                snprintf(mini_dig_ips[found_ip_count]
                        + strlen(mini_dig_ips[found_ip_count]),
                        elemSize, "%u.", c);
                i++;
                c = recv_buf[i];
                snprintf(mini_dig_ips[found_ip_count]
...
int MiniDigIptablesAdd(char *string, int count) {
    ...
    snprintf(buffer, sizeof(buffer), "%s%s", __MINI_DIG_STORE_LOCATION__, string);
    ...
    if (count < 0) {
        ...
        while (i < count) {
            ...
            memset(buffer, 0x00, sizeof(buffer));
            snprintf(buffer, sizeof(buffer), "iptables -w -I INPUT 1 -p tcp -s %s/32 -j DROP",
                    mini_dig_ips[i]);
            system(buffer);

            // DNS(port 53)
            memset(buffer, 0x00, sizeof(buffer));
            snprintf(buffer, sizeof(buffer), "iptables -w -I INPUT 1 -p udp --dport %d -s %s/32 -j DROP",
            ...
            memset(buffer, 0x00, sizeof(buffer));
            snprintf(buffer, sizeof(buffer), "iptables -w -I FORWARD 1 -p tcp -s %s/32 -j DROP",
                    mini_dig_ips[i]);
            system(buffer);

            // DNS(port 53)
            memset(buffer, 0x00, sizeof(buffer));
            snprintf(buffer, sizeof(buffer), "iptables -w -I FORWARD 1 -p udp --dport %d -s %s/32 -j DROP",
                    __MINI_DIG_DNS_PORT__, mini_dig_ips[i]);
            system(buffer);

            snprintf(buffer, sizeof(buffer), "%s%c", mini_dig_ips[i], '#');
    ...
    if (file_size > 15) {
        ...
        while (1) {
            if (buffer[i] == '#') {
                ...
                snprintf(cmd_buf, sizeof(cmd_buf), "iptables -w -D INPUT -p tcp -s %s/32 -j DROP", ip);
                system(cmd_buf);

                memset(cmd_buf, 0x00, sizeof(cmd_buf));
                snprintf(cmd_buf, sizeof(cmd_buf), "iptables -w -D INPUT -p udp --dport %d -s %s/32 -j DROP", __MINI_DIG_DNS_PORT__, ip);
                system(cmd_buf);

                // for other traffics(TCP, DNS)
                memset(cmd_buf, 0x00, sizeof(cmd_buf));
                snprintf(cmd_buf, sizeof(cmd_buf), "iptables -w -D FORWARD -p tcp -s %s/32 -j DROP", ip);
                system(cmd_buf);

                memset(cmd_buf, 0x00, sizeof(cmd_buf));
                snprintf(cmd_buf, sizeof(cmd_buf), "iptables -w -D FORWARD -p udp --dport %d -s %s/32 -j DROP", __MINI_DIG_DNS_PORT__, ip);
                system(cmd_buf);
